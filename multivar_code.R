...
# coveriance ellipse plot table
    ...
    ggplot(scrs, aes(x = NMDS1, y = NMDS2))+
      ...
      #xlim(-1,0.6)+
      #ylim(-1,0.7)+
      annotate("text", x = 0.38, y = -0.75,label= paste("Stress =",round(nmds$stress,digits=3)), 
               fontface="italic")
...
# ADONIS (PERMANOVA) ----  
    ...
    ENV$region2<- as.factor(c(rep(1, 15),rep(2, 12),rep(1, 4), rep(3, 11),rep(4, 21), rep(5, 15)))
    
    
    PA.adonis<-adonis(PA ~ ENV$region2, permutations=999, method = "bray")
    PA.adonis<-adonis(PA ~ ENV$station, permutations=999, method = "bray")
    PA.adonis<-adonis(PA ~ ENV$seep, permutations=999, method = "bray")
     
    print(PA.adonis)
    densityplot(permustats(PA.adonis))    
